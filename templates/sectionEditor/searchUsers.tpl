...
<form id="submit" method="post" action="{url op="enrollSearch" path=$articleId}">
	...
	<input type="text" size="15" name="search" class="textField" value="{$search|escape}" />&nbsp;<input type="submit" value="{translate key="common.search"}" class="btn changeRadius marginTopSm" />
</form>
...
<div class="row">
  <div class="col-xs-12">
    <input type="submit" value="{translate key="manager.people.enrollSelected"}" class="btn btn-block marginTopBot" id="regSubmitBtn" />
  </div>
  <div class="col-xs-12">
      <input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-block cancelBtn" onclick="document.location.href='{url page="manager" escape=false}'" />
  </div>
</div>
