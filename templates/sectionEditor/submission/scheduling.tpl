	...
	<form action="{url op="scheduleForPublication" path=$submission->getId()}" method="post">
		<tr valign="top">
			...
			<td width="50%" class="value">
				<input type="submit" value="{translate key="common.record"}" class="btn changeRadius marginLeftSm" />&nbsp;
				{if $issueId}
					{if $isEditor}
...
{else}
	<tr>
		...
		<td align="right">
			...
			<input type="submit" value="{translate key="payment.paymentReceived"}" class="btn changeRadius marginLeftSm" />&nbsp;
			</form>
		</td>
		{if $isEditor}
			<td align="left">
				<form action="{url page="editor" op="waivePublicationFee" path=$submission->getId()}" method="post">
					<input type="hidden" name="sendToScheduling" value=1 />
					<input type="submit" value="{translate key="payment.waive"}" class="btn changeRadius marginLeftSm" />&nbsp;
				</form>
			</td>
		{/if}
	</tr>
